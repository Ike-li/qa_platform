...
> 试点产品边界、Beta/GA 准入和成功指标见 [`product.md`](product.md) §9
> **首批可交付任务包见 [`tasks/`](tasks/README.md)**（codex-ready，含规格 + 起点 + 验收 + 约束）；本轮审计新增的待办若进入实施，需要后续补任务包。
> 本文件按优先级排序验收状态、剩余缺口与不做范围，与 catalog §4 保持同步。
> 更新于 2026-06-07（PRD / catalog / tasks 与代码的历史偏移审计见 [`archive/doc-conflict-audit.md`](archive/doc-conflict-audit.md)；旧 `feature/T*` 分支合并状态仅作为审计快照保留，当前实现与 backlog 以本文件、catalog 和现场 git 状态为准）
...
## 1. 高优先级 — PRD 验收状态与剩余必做项

| # | 项 | 来源 | 状态 / 剩余缺口 |
|---|---|---|---|
| 1 | F-PL-02 环境变量加密 | PRD §3.2 验收 | 已完成：`env_vars` 以 JSON-safe AES-256-GCM envelope 存入 JSONB，API create/fetch/update、AAD 错配失败 audit、migration 加密既有数据、worker 解密注入容器环境变量均有自动化证据；nightly/manual external-stack 还断言 worker 注入密钥不会出现在平台产出的 artifact 元数据、下载内容或归档日志回看中；用户测试进程主动把密钥打印到 stdout/stderr 后，executor 写 Redis 前会按环境变量敏感值脱敏，live SSE、archived logs 与真实 `run.read` token 读面只出现 `[REDACTED]` |
...
| 5 | 审计写入覆盖补齐 | architecture §9.6 | 已覆盖：single/batch cancel、batch retry、SSE ticket、auth 失败审计 outage、audit-events 查询成功/拒绝副作用、projects/members/pipelines/credentials/environments/notification rules/schedules、schedule worker 自动触发与 missing-pipeline skip、签名 webhook `run.trigger` 与 filtered/duplicate 决策 audit 均已补真实 DB 验证；审计不泄露 password/token/secret，敏感字段脱敏或保留 delete before_state；API POST/PUT/PATCH/DELETE 路由由 `tests/unit/test_architecture_boundaries.py::test_mutating_api_routes_keep_audit_write_contract` 锁住。逐项断言见 `testing-strategy.md` 风险矩阵；后续新增/变更写路径必须同步审计与脱敏回归 |
| 6 | F-PL-03 / F-RE-04 产物限制与上传/预览闭环补齐 | PRD §3.2 / PRD §3.4 验收 | 已完成基线：CPU/内存/超时、产物数量/大小限制、`disk_mb` → Docker `StorageOpt.size`、`results/` 递归上传与 Allure 目录文件落库均已实现；OOM/timeout 归一到 Run `timeout` 并采样 Docker stats 峰值；前端 Allure HTML 预览有 E2E，普通 HTML/report 预览由前端契约测试覆盖。RBAC/scope/跨租户/presign 副作用、worker 黑盒与 OOMKilled gate 的完整测试证据见 `testing-strategy.md` 风险矩阵。**仍缺多资源报告加载口径** |
| 7 | F-EX-05 日志归档回看闭环 | PRD §3.3 验收 | 已完成：Redis Stream 实时日志/状态事件、SSE `Last-Event-ID` 断点续传、S3 JSONL 归档写入、归档失败 Redis retry marker + worker cron 重试、归档日志读回 API（分页/大页只读对象不 presign、对象缺失 404、存储未配置 503）均已实现；前端终态 run 已接入归档日志 API 并由真实 DB+S3 E2E 覆盖回放/搜索。`run.read` scope、跨租户 404、密钥脱敏与 worker 黑盒证据见 `testing-strategy.md` 风险矩阵。剩余增强是大日志虚拟列表体验和对象存储异常可观测性 |
...
| 11 | F-NT-02 多渠道通知 | PRD §8 | 已完成：Email、Webhook、DingTalk 自定义机器人、WeCom 群机器人均已实现；前端通知规则表单可创建四类渠道，email 会按后端发送契约保存 `smtp_host/from_address/to_addresses`，DingTalk/WeCom 密钥字段只在表单输入中使用，列表卡片不渲染密钥；API 与前端均禁止单条规则重复配置同一渠道类型，避免通知日志按类型幂等时静默少发 |

## 2. 中优先级 — 验收状态与增强缺口

| # | 项 | 来源 | 状态 / 剩余缺口 |
|---|---|---|---|
| 12 | F-EX-02 静默窗口 | PRD §3.3 验收 | 已完成：项目级 `silent_windows` 通过 `PUT /api/v1/projects/{project_id}` 写入 `Project.settings`，前端项目设置页可编辑；cron 命中发布冻结期时不创建 Run、不更新 `last_run_at`，并写 `schedule_skipped_silent_window` audit；手动触发和 webhook 触发明确忽略 silent_windows；required integration 覆盖真实 API/DB 保存、窗口内/外 cron 和 manual/webhook 绕过 |
| 13 | F-EX-03 Webhook Git 平台事件解析 | PRD §3.3 验收 | GitHub signed push provider 入口已补：`/webhooks/github` 与 `/api/v1/webhooks/github` 会按 repository URL candidates 匹配项目，用匹配项目 `webhook_secret` 验签，并以系统身份创建 Run / filtered audit；项目级 webhook 继续覆盖 HMAC、branch filter、dedup 与终态同 commit 再触发；剩余是 GitLab/Gitee provider、PR/fork 策略、URL 规范化/歧义运营提示和更完整事件矩阵 |
| 14 | F-EX-07 自动重试端到端补齐 | PRD §3.3 验收 | API-facing `max_attempts` / `retry_on`、waiting retry run、execute_run 基础设施异常、真实 `RunExecutor` setup Docker daemon `ConnectionError` retry、setup script 非 0 与 git clone 失败不误 retry、worker_lost reclaimer 的真实 DB/Redis status event `previous=running`、orphan cleanup 与 retry Run 入队已进 required integration；nightly/manual external-stack 已补 worker_lost 黑盒重试，以及 credentialed clone failure / setup exit 1 不 retry、不落 artifact、不泄密黑盒，其中 clone failure 会回看 archived logs API 并断言空或非空响应都不含 secret/userinfo；剩余增强是把 Docker daemon 扰动扩到 external-stack 黑盒 |
| 15 | F-EX-08 优先级队列消费闭环 | PRD §3.3 验收 | 已补 compose high/medium/low worker 部署、manual priority 队列矩阵单测、真实 API/DB queue metadata 测试、真实 DB priority+FIFO 排序测试、nightly/manual performance smoke 覆盖 `dequeue_waiting` 恢复 waiting Run 入队，以及容量受限长队中 newer high priority 越过 older low backlog 的 priority preemption smoke；nightly/manual external-stack 已覆盖 high/low worker 队列隔离黑盒；真实长队公平性/抢占的更大规模压测仍可作为专项增强 |
| 16 | F-LS-04 测试结果 suite/关键字过滤 | PRD §3.7 验收 | 已完成：`/runs/{run_id}/results` 支持 `status` / `suite` / `q` 组合过滤，`q` 覆盖用例名和错误信息并转义 LIKE 通配符；unit 覆盖过滤条件，required integration 覆盖真实 DB 行组合过滤和重复约束 rollback 后恢复查询 |
| 17 | F-LS-01 执行列表过滤补齐 | PRD §3.7 验收 | 已完成：`/runs` 支持 status 多选、project_id、pipeline_id、git_ref、created_from/created_to 和创建时间排序；required integration 覆盖真实 DB 行的 pipeline/git_ref/time range 过滤 |
| 18 | F-LS-02 剩余列表分页补齐 | PRD §3.7 验收 | 已完成：credentials、project members、auth tokens 均返回 `PaginatedResponse`，支持 `page` / `per_page` / `total` / `data`，`per_page <= 100`；unit 覆盖 offset/limit 传递，required integration 覆盖真实 DB/API 分页、越界页空数组和 token 明文不回显 |
| 19 | F-LS-03 项目搜索排序补齐 | PRD §3.7 验收 | 已完成：`/api/v1/projects` 支持 `q` 按 name/description 模糊搜索并转义 LIKE 通配符，结果按项目名称升序；unit 锁住 repository `order_by`，required integration 覆盖真实 DB/API 下乱序创建后的搜索结果名称字母序 |
| 20 | F-RE-05 单用例历史趋势补齐 | PRD §3.4 验收 | 已完成：`/api/v1/projects/{project_id}/analytics/test-history` 按 suite/name 返回单用例历史点，包含 run id、run 创建时间、run 终态、用例状态、耗时、错误信息与 git ref；同时按 product.md §9 补齐轻量 release 判断，trends/flaky 支持可选 `git_ref`，`/analytics/release-summary` 返回目标 ref 与 baseline 的 run 数、raw pass rate、flaky-adjusted pass rate、新增失败和恢复用例；Analytics 面板可从 flaky 行或失败 Run 查看单用例历史。required integration 覆盖真实 DB 下 passed/failed 两次历史、pagination、run_status、git_ref 与 duration/error；前端契约、build 和 lint 已覆盖 DTO/hook/UI 入口 |
...
| 22 | 非功能性能压测 | PRD §4 / PRD §3.4 验收 | 已补 nightly/manual performance smoke：覆盖读/写 API、手动/webhook/schedule 触发入队、waiting dequeue 恢复、取消 API p99、SSE 推送 < 2s、真实 API token 并发读、归档/artifact 读面副作用、audit-events 查询、执行摘要 < 3s 和 external-stack worker 单样本/10 容器并发 SLO；release_candidate 按 `.github/performance-slo-manifest.json`/`baseline.json` 校验并阻断基线退化。逐项 SLO 与拒绝路径断言见 `testing-strategy.md` Performance smoke 小节；完整容量压测和长期稳定性 SLO 仍需专项环境验证 |
| 23 | E2E CI 覆盖扩展 | archive/fix-roadmap.md §4.3 | 已补三档 gate：PR/pr_like 跑轻量 `auth-flow`，nightly/schedule 跑固定真实 E2E（`real-login-flow`、`real-run-trigger`、`special-regressions`），release_candidate 跑全量 Playwright 且强制四个关键 spec 存在；nightly/release_candidate 设置 `QAP_E2E_WORKER=1` 启动受控 worker，`real-run-trigger` 经 UI modal 触发真实 run 并验证 worker 终态/归档日志/JUnit/artifact/页面；本地 `scripts/run-e2e.sh` 与 `global-setup.ts` 对齐 seed/登录口径。逐项 evidence validation 见 `testing-strategy.md` E2E 小节 |
...
| # | 项 | 备注 |
|---|---|---|
| 25 | OpenTelemetry 装配 | 基础装配已补：`observability/tracing.py`、API/worker instrumentation 和 worker/executor 手动 span 均已落地；剩余是 OTLP HTTP exporter 依赖决策、接收端部署验证和 trace-log 关联后续优化 |
| 26 | 部署 checklist 完善 | 密钥/CIDR/lifecycle/Docker socket proxy 一键勾选；附到 `runbook.md` |
...
| `T-NOTIFICATION-CONDITION` | 已完成：F-NT-01 条件通知验收补齐 |
| `T-RETENTION-OPS` | 数据保留冷归档/读回增强 |
| `T-LINT` | 技术债专项 |
| `T-FRONTEND-TS` | 已完成：清理前端历史 TS build 债 |
...
| F-EX-08 是否追加更重的长队公平性压测 | 已按多 worker 部署落地 compose high/medium/low，并有 external-stack 队列隔离黑盒；performance smoke 已覆盖容量受限 priority backlog 中 high 越过 older low |
...
| F-EX-08 采用单 worker 多队列还是多 worker 部署 | 已按多 worker 部署落地 compose high/medium/low，并有 external-stack 队列隔离黑盒 |
| F-NT-01 是否支持 OR / 连续失败条件 | 已支持：`any` / `all` 条件组与 `consecutive_failures` |
...
- 完整跨分支/跨环境对比专属视图、历史日志全文搜索、数据导出 CSV、Slack 通知；轻量 git_ref release 判断已进入 Analytics 范围
...
| 阶段 | 状态 |
|---|---|
| Phase 1 MVP | ⚠️ 主线部分完成（项目/管道/手动执行/日志/结果主链路、F-PL-01 collector 配置、F-PL-02 env_vars 加密、F-PM-01/02 Git 凭证 clone 基线和 F-LS-01/02/03/04 列表验收已就位；F-PL-03 多资源报告加载体验仍缺） |
| Phase 2 自动化与通知 | ⚠️ 主线部分完成（cron/webhook/API token 基础、重试/优先级队列原语、通知规则/模板基础已就位；API token scope、webhook 分支过滤+去重、GitHub push provider repo 匹配、自动重试主干、优先级队列主干、DingTalk/WeCom 通知渠道、通知条件 OR/连续失败和项目级静默窗口均已有测试证据；F-EX-03 更完整 Git provider 事件矩阵仍缺） |
| Phase 3 洞察与报告 | ⚠️ 主线部分完成（仪表盘/Flaky/系统状态页/项目级趋势和单用例历史趋势已就位；多资源报告加载体验仍缺） |
