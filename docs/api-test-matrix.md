...
| 状态 | 数量 | 含义 |
| --- | ---: | --- |
| `covered` | 56 | `/health`、`/ready`、Auth、Project CRUD、project members list、project resources CRUD、audit-events，以及公开 API 可构造的 runs、SSE、webhook operation 已有黑盒行为测试。 |
| `partial` | 0 | 不再保留未决 partial；不可稳定构造的 success 路径统一进入阻塞清单。 |
| `blocked_by_missing_public_setup` | 13 | success 路径需要无法通过公开 API 创建的前置状态，已记录阻塞原因。 |
...
| `covered` | 401 | 已由 contract smoke、黑盒行为测试或声明状态校验覆盖/守护的原子测试点。 |
| `blocked_by_missing_public_setup` | 13 | 仅 success 维度阻塞；对应下方阻塞清单，不能计入已完成成功态。 |
...
| 合计 | 340 | `RUN_INTEGRATION_TESTS=1 ... --collect-only -q` 当前收集结果。 |
...
| `GET /api/v1/artifacts/{artifact_id}/download` | 200 path 需要已持久化 artifact 对象；公开 API 不能直接上传 worker artifact 对象。 |
| `GET /api/v1/artifacts/{artifact_id}/preview-url` | 200 path 需要可预览 artifact 对象；公开 API 不能直接创建该对象。 |
| `GET /api/v1/artifacts/{artifact_id}/preview/{token}/{artifact_path}` | 需要有效 preview token 和 artifact 对象；没有公开 setup API。 |
| `POST /api/v1/projects/branches` | 200 path 需要可访问的 public/SSH Git 远程仓库；本地 `file://` 仓库被公开 schema 拒绝，外网 Git fixture 不适合作为稳定黑盒门禁。 |
| `GET /api/v1/projects/{project_id}/analytics/trends` | 已覆盖空结果、参数校验和租户隔离；有数据趋势需要 worker/result ingestion 写入完整结果数据，缺公开 setup API。 |
| `GET /api/v1/projects/{project_id}/analytics/flaky` | 已覆盖空结果、参数校验和租户隔离；有数据 flaky 统计需要 TestResult 行，缺公开 setup API。 |
| `GET /api/v1/projects/{project_id}/analytics/test-history` | 已覆盖空结果、参数校验和租户隔离；有数据历史需要 TestResult 行，缺公开 setup API。 |
| `GET /api/v1/runs/{run_id}/artifacts/allure-report` | 200 path 需要 worker 生成并持久化 Allure report artifact；没有公开 setup API。 |
| `GET /api/v1/runs/{run_id}/logs/archive` | 200 path 需要 worker/S3 归档日志对象；没有公开 setup API。 |
...
| Runs / Logs / Artifacts | 14 | 触发运行、批量取消/重试、查询运行、结果、通知、SSE、终态幂等、跨租户不可见已覆盖；Allure artifact 与归档日志成功态为阻塞项。 |
| Analytics / Audit | 4 | audit-events 的分页、actor/resource/time 过滤、schema negative、跨租户不可见已覆盖；analytics 的空结果、参数校验和跨租户隔离已覆盖，有数据成功态为阻塞项。 |
...
.venv/bin/python -m ruff check scripts/export_openapi.py scripts/validate_api_test_matrix.py scripts/validate_api_test_case_matrix.py tests/integration/test_openapi_contract_smoke.py tests/integration/test_openapi_behavior_blackbox.py tests/integration/conftest.py tests/unit/test_release_gate_workflow.py tests/unit/test_api_test_matrix_validator.py tests/unit/test_api_test_case_matrix_validator.py
```
...
3. 行为测试不得读取数据库、仓储实现或业务内部 helper 来构造断言；只允许通过公开 HTTP API 搭建、查询和清理数据。
