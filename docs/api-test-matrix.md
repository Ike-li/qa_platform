...
| 状态 | 数量 | 含义 |
| --- | ---: | --- |
| `covered` | 78 | `/health`、`/ready`、Auth、Project CRUD、project members、project resources CRUD、audit-events、公开 API 可构造的 runs/SSE/webhook operation、report sharing、外部结果导入（runs/import）、失败分诊（runs/triage）、失败子集重跑（runs/retry-failed）、Flaky 隔离（projects/quarantine）、real-stack 可构造的 artifact/archived logs/analytics 有数据成功态，以及 seed-assisted admin/branches/member 写操作成功态已有黑盒行为测试。 |
...
| 状态 | 数量 | 含义 |
| --- | ---: | --- |
| `total_cases` | 468 | 78 个 operation × `contract`、`auth`、`success`、`schema_negative`、`rbac_tenant`、`declared_responses` 6 个维度。 |
| `covered` | 468 | 已由 contract smoke、黑盒行为测试、seed-assisted 黑盒行为测试、real-stack 行为测试、专项集成测试（`tests/integration/test_run_import.py`、`tests/integration/test_run_triage.py`、`tests/integration/test_run_retry_failed.py`）或声明状态校验覆盖/守护的原子测试点。 |
...
| 合计 | 362 | `RUN_INTEGRATION_TESTS=1 ... --collect-only -q` 当前收集结果（另有 `tests/integration/test_run_import.py` 7 个导入专项用例、`tests/integration/test_run_triage.py` 5 个分诊专项用例）。 |
...
| Runs / Logs / Artifacts | 16 | 触发运行、批量取消/重试、查询运行、结果、通知、SSE、终态幂等、跨租户不可见已覆盖；real-stack 行为测试覆盖 artifact download、preview、Allure 入口与归档日志成功态；外部结果导入（`POST /projects/{project_id}/runs/import`，导入不去重、重复上传生成独立 Run）的 success/422/413/跨租户/API token 场景由 `tests/integration/test_run_import.py` 覆盖；失败分诊（`GET /runs/{run_id}/triage`，三类归类/签名聚类/履历/性能粗断言）的 success/空态/跨租户场景由 `tests/integration/test_run_triage.py` 覆盖。 |
