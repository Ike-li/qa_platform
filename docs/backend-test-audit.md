...
- CI 稳定性：后端 ruff 与单测覆盖率是同一门禁；PR/push 必跑 required integration；heavy Docker/worker integration 拆到 nightly/manual；external-stack preflight 要求 QA Platform `/ready` JSON，nightly/manual 通过 `QAP_EXTERNAL_STACK_REQUIRED=1` 把 API/worker 栈不 ready 变成失败，本地未启动或 8000 被其它服务占用时保持前置条件 skip；heavy Docker fixture 先复用本地镜像、缺镜像才 pull，避免 registry/network 抖动制造误 skip；backend CI 会上传 unit/required/heavy/external-stack/performance 的 JUnit 与 pytest log，单元覆盖率还上传 XML/JSON，external-stack 失败时上传 Docker compose PostgreSQL/Redis/MinIO/API/worker ps/log，performance smoke 上传每个 SLO 断言的 p50/p99/max JSONL 和 Markdown 摘要；evidence manifest 会校验当前 lane 期望证据文件非空；PR E2E 保留轻量 UI 冒烟，nightly 固定跑真实 E2E 主路径，完整真实 E2E 留给手动 workflow。
...
- `backend-test`：先跑 `ruff check src tests`，再跑后端单元测试和 coverage fail-under；上传 JUnit、pytest log、coverage XML/JSON。
