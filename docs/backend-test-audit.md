...
- 真实 worker 黑盒：nightly/manual 会启动 compose API/worker/MinIO；external-stack smoke 覆盖真实 API 触发后 worker 执行容器、多 artifact 列表、逐个预签名下载链接、真实 MinIO 对象内容下载、归档日志 API、worker_lost 后自动 retry 再完成，以及停掉 high/low worker 后 high/low priority run 不被 medium worker 误消费、启动匹配 worker 后完成的队列隔离链路。
...
- integration 收集：148 tests
- PR/push 必跑 required integration：121 passed, 27 deselected
- 完整 local integration（未启动外部 API/worker 栈）：129 passed, 19 skipped
