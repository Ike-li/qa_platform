...
- 非功能 smoke：nightly/manual 覆盖读 API、写 API、触发入队 SLO、取消 API p99、Redis 日志写读、SSE 实时日志推送 < 2s、归档日志读回 API（小样本、1500 行大对象分页与跨租户拒绝不读 S3）、artifact 列表元数据 API、artifact 下载链接 API（单次、burst 与跨租户拒绝不 presign）、audit events 查询 API、audit-events 拒绝查询不写自审计、执行摘要生成 < 3s 趋势哨兵，并在失败时输出 p50/p99/max 摘要；不把性能环境抖动放进 PR 硬门禁。
...
- integration 收集：154 tests
- PR/push 必跑 required integration：123 passed, 31 deselected
- 完整 local integration（未启动外部 API/worker 栈）：131 passed, 23 skipped
- performance smoke opt-in：16 passed
