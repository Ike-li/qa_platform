...
- Worktree currently contains API-test hardening changes plus other in-progress source/test updates; do not assume a clean tree during handoff.
...
- OpenAPI interface testing is at full matrix coverage:
  ...
  - Pytest collection for the OpenAPI API-test slice is 352 items.
  - CI writes API test quality evidence to `artifacts/backend-integration/api-test-quality/summary.json` and `artifacts/backend-integration/api-test-quality/index.md`.
...
- `tests/integration/test_openapi_real_stack_behavior.py`: compose API/worker/MinIO black-box workflow split into run summary, artifact/preview, archived logs, and analytics assertions.
- `scripts/report_api_test_quality.py`: API test quality dashboard/report generator backed by the operation and case matrix validators.
...
.venv/bin/python -m pytest tests/unit/test_api_test_matrix_validator.py tests/unit/test_api_test_case_matrix_validator.py tests/unit/test_api_test_quality_report.py tests/unit/test_frontend_api_contract.py tests/unit/test_release_gate_workflow.py -q
RUN_INTEGRATION_TESTS=1 .venv/bin/python -m pytest tests/integration/test_openapi_contract_smoke.py -q
RUN_INTEGRATION_TESTS=1 .venv/bin/python -m pytest tests/integration/test_openapi_behavior_blackbox.py::test_webhooks_blackbox_security_failures_and_duplicate_delivery -q
...
- API test quality report generated `summary.json` and `index.md`.
- Related unit tests: 60 passed.
- Behavior blackbox webhook security flow: 1 passed.
- Contract smoke: 213 passed.
- Real-stack behavior: 4 passed.
- Collect-only: 352 tests collected.
