...
| Worker/queue 状态机与产物链路 | 真实 PG repository 状态机、run claim、cancel、state visibility、execute_run 基础设施异常重试、executor/task 单测、artifact size/count/recursive upload、S3 上传失败无孤儿 DB 行、下载链接 bucket/key/TTL 真实 DB/API 测试、归档日志默认页/分页/缺失对象/跨租户 404 API 测试、heavy Docker cancel、compose high/medium/low worker 配置、external-stack worker smoke 和 worker_lost retry 黑盒 | 已覆盖 queued/preparing/running/collecting/done/failed/cancelled/timeout；priority+FIFO 排序、worker retry 落库、artifact 递归落库、失败上传不落库和下载链接参数有真实 DB/API/RBAC 测试；nightly/manual 真实外部栈覆盖 worker 产物内容下载、归档日志读回和 worker_lost 后自动 retry |
