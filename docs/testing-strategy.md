...
| Worker/queue 状态机与产物链路 | 真实 PG repository 状态机、run claim、cancel、state visibility、execute_run 基础设施异常重试、RunExecutor OOM/timeout 资源终止写真实 DB/Redis、manual priority 到 `queue:high/medium/low` 的真实 API/DB 入队元数据、executor/task 单测、cancel API 真实 Redis status event `previous` 与 p99 smoke、artifact size/count/recursive upload、S3 上传失败无孤儿 DB 行、artifact 列表元数据、下载链接 bucket/key/TTL/API token scope/跨租户 404 真实 DB/API 测试、归档失败真实 Redis retry marker/TTL + worker cron 重试、归档日志默认页/分页/缺失对象/API token scope/跨租户 404 API 测试、heavy Docker cancel/OOM、compose high/medium/low worker 配置、external-stack worker smoke 和 worker_lost retry 黑盒 | 已覆盖 queued/preparing/running/collecting/done/failed/cancelled/timeout；priority+FIFO 排序、manual priority 队列元数据、worker retry 落库、`oom_killed` / `timed_out` backend 结果到 Run `timeout` 终态和 Redis status event、artifact 递归落库、失败上传不落库、归档失败补偿、cancel 控制面的 Redis previous/audit 前后态、API p99 趋势和 artifact 列表/下载链接参数有真实 DB/API/RBAC/Redis 测试；artifact 元数据列表、预签名 URL 与归档日志读回都需要 `run.read` scope，跨租户 artifact ID 与随机 ID 同样 404；nightly/manual 真实外部栈覆盖 worker 解密环境变量后注入容器、多 artifact 内容下载、归档日志读回和 worker_lost 后自动 retry |
