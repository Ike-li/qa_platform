...
| Performance smoke | nightly/manual 跑 `tests/integration/test_performance_smoke.py -m performance` | 读 API、写 API、触发入队 SLO（每次采样都写 `run.trigger` 审计且状态字段一致）、取消 API p99、Redis 日志写读、SSE 实时日志推送 < 2s、归档日志读回 API（小样本与 1500 行大对象分页均只读 `logs/{run_id}.jsonl` 且不 presign，对象缺失稳定 404 且只读目标归档对象，跨租户拒绝不读 S3）、artifact 列表元数据 API（成功路径 DB-only 不 presign/读 S3）、artifact 列表拒绝不返回元数据、artifact 下载链接 API（单次/burst 成功路径 presign-only 不读对象，跨租户拒绝不 presign）、audit events 查询 API 与成功自审计写入、audit-events 拒绝查询不写自审计、执行摘要生成 < 3s 的趋势哨兵；失败时输出 p50/p99/max 摘要 | 不是 PR 阻塞性 SLO 证明；发现退化后升格专项压测 |
...
| 风险面 | 当前证据 | 状态 |
| --- | --- | --- |
| RBAC：tenant role、project role、platform admin、API token scope | audit events、cross-tenant API、SSE 403/404、SSE ticket + `Last-Event-ID` 真实续传、artifact list/download 与 archived logs 的 `run.read` scope、真实 JWT/API token integration、auth 失败审计 outage 与 refresh revoke Redis blip 回归、register/refresh/logout/API token 审计防泄密 | 已覆盖主干；API token scope 已补真实路由矩阵，只读/错误/空 scope 都会被门禁，artifact 元数据列表、预签名 URL 和归档日志读回都不能由 `project.read` token 换取，tenant A `run.read` token 不能枚举 tenant B artifact；audit-events 成功查询写 `audit_events.list` 自审计，member/viewer 403 和跨租户过滤 404 不写误导性自审计，nightly/manual 还观察拒绝查询 p99；login/refresh/logout 在审计写入异常下不把 401/204 变成 500，refresh revoke Redis 抖动不阻断 token 轮换；register/refresh/logout 不把 password/access token/refresh token 写入 AuditEvent before/after，API token 创建/撤销审计只记录 name/scopes 等可追溯元数据 |
