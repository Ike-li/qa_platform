...
| Required integration | `backend-integration-test` 在 PR/push 跑 `tests/integration -m "not heavy_docker and not external_stack and not performance"` | 真实 PostgreSQL/Redis/Testcontainers、FastAPI 路由、repository 约束、API 写入后的 DB 状态、RBAC/租户隔离、worker 入口异常重试、SSE `Last-Event-ID` 断点续传、归档日志读回分页/对象缺失、artifact 限制落库与下载链接 bucket/key/TTL、签名 webhook 的 HMAC/branch filter/dedup/audit/保留 metadata 保护 | Docker 容器执行、外部 API+worker compose 栈、平台相关 OOM 语义与 performance smoke |
...
| Webhook/schedule 幂等与失败路径 | HMAC 缺失/成功签名、branch filter、dedup、terminal same commit、cross-tenant 404、archived project、missing pipeline/environment、enqueue conflict、保留 metadata 防覆盖、`run.trigger` audit | Webhook/API/schedule worker 主风险已进 required integration；schedule pipeline missing 仍由 unit 覆盖，因为真实 DB FK 会把硬删除变成级联，软删除路径不等价于真实缺行 |
