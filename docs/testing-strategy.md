...
| 数据隔离：跨 tenant/project、soft-delete、随机 UUID | `test_cross_tenant_isolation.py`、repository/API 真实 DB 测试、artifact/result/归档日志隐藏测试、真实 API token 跨租户 artifact 下载 404 收敛、artifact 拒绝下载 no-presign p99 smoke | 已作为 PR integration 门禁；Member/Viewer 项目级路由先归一 404 再做 RBAC；artifact 下载对跨租户真实 ID 与随机 UUID 不暴露差异且不调用 S3 presign，nightly/manual 还观察该拒绝路径的 p99 |
