...
| Required integration | `backend-integration-test` 在 PR/push 跑 `tests/integration -m "not heavy_docker and not external_stack and not performance"` | 真实 PostgreSQL/Redis/Testcontainers、FastAPI 路由、repository 约束、API 写入后的 DB 状态、RBAC/租户隔离、worker 入口异常重试、schedule worker 自动触发 audit、SSE `Last-Event-ID` 断点续传、归档日志读回分页/对象缺失、artifact 限制落库与下载链接 bucket/key/TTL、签名 webhook 的 HMAC/branch filter/dedup/`run.trigger` audit、filtered/duplicate 决策 audit、保留 metadata 保护 | Docker 容器执行、外部 API+worker compose 栈、平台相关 OOM 语义与 performance smoke |
...
| Performance smoke | nightly/manual 跑 `tests/integration/test_performance_smoke.py -m performance` | 读 API、写 API、触发入队 SLO、Redis 日志写读、SSE 实时日志推送 < 2s、归档日志读回 API、artifact 下载链接 API、执行摘要生成 < 3s 的趋势哨兵；失败时输出 p50/p99/max 摘要 | 不是 PR 阻塞性 SLO 证明；发现退化后升格专项压测 |
...
| API 写入后的真实 DB 状态 | projects、project members、pipelines、credentials、environments、notification rules、runs、schedules、auth/token、batch cancel/retry audit 链路 | 已补 required integration；项目 `git_url` userinfo、凭据明文、环境变量值、通知 channel/webhook/template 不落 audit before/after，project/pipeline/member/notification/schedule delete 有删除前快照 |
| Worker/queue 状态机与产物链路 | 真实 PG repository 状态机、run claim、cancel、state visibility、execute_run 基础设施异常重试、executor/task 单测、artifact size/count/recursive upload 与下载链接 bucket/key/TTL 真实 DB/API 测试、归档日志默认页/分页/缺失对象 API 测试、heavy Docker cancel、compose high/medium/low worker 配置、external-stack worker smoke 和 worker_lost retry 黑盒 | 已覆盖 queued/preparing/running/collecting/done/failed/cancelled/timeout；priority+FIFO 排序、worker retry 落库、artifact 递归落库和下载链接参数有真实 DB/API/RBAC 测试；nightly/manual 真实外部栈覆盖 worker 产物内容下载、归档日志读回和 worker_lost 后自动 retry |
| Webhook/schedule 幂等与失败路径 | schedule worker 成功触发与 enqueue conflict 的系统 `run.trigger` audit、HMAC 缺失/成功签名、branch filter、dedup、filtered/duplicate 决策 audit、terminal same commit、cross-tenant 404、archived project、missing pipeline/environment、enqueue conflict、保留 metadata 防覆盖 | Webhook/API/schedule worker 主风险已进 required integration；filtered/duplicate 不创建 Run 的分支已有项目级 AuditEvent 且 after_state 不含 repo URL、dedup_key 或任意 metadata；schedule pipeline missing 仍由 unit 覆盖，因为真实 DB FK 会把硬删除变成级联，软删除路径不等价于真实缺行 |
