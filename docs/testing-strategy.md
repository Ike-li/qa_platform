...
| Required integration | `backend-integration-test` 在 PR/push 跑 `tests/integration -m "not heavy_docker and not external_stack and not performance"` | 真实 PostgreSQL/Redis/Testcontainers、FastAPI 路由、repository 约束、API 写入后的 DB 状态、RBAC/租户隔离、worker 入口异常重试、schedule worker 自动触发 audit 与 missing-pipeline skip audit、SSE `Last-Event-ID` 断点续传、归档失败真实 Redis retry marker/worker cron 重试、归档日志读回分页/对象缺失和跨租户 404、artifact 限制落库、S3 上传失败无孤儿 Artifact 行、下载链接 bucket/key/TTL、artifact 下载 API token `run.read` scope、签名 webhook 的 HMAC/branch filter/dedup/`run.trigger` audit、filtered/duplicate 决策 audit、保留 metadata 保护 | Docker 容器执行、外部 API+worker compose 栈、平台相关 OOM 语义与 performance smoke |
...
| Performance smoke | nightly/manual 跑 `tests/integration/test_performance_smoke.py -m performance` | 读 API、写 API、触发入队 SLO、Redis 日志写读、SSE 实时日志推送 < 2s、归档日志读回 API、artifact 下载链接 API、audit events 查询 API、执行摘要生成 < 3s 的趋势哨兵；失败时输出 p50/p99/max 摘要 | 不是 PR 阻塞性 SLO 证明；发现退化后升格专项压测 |
...
| 风险面 | 当前证据 | 状态 |
| --- | --- | --- |
| RBAC：tenant role、project role、platform admin、API token scope | audit events、cross-tenant API、SSE 403/404、SSE ticket + `Last-Event-ID` 真实续传、artifact download `run.read` scope、真实 JWT/API token integration | 已覆盖主干；API token scope 已补真实路由矩阵，只读/错误/空 scope 都会被门禁，artifact 预签名 URL 不能由 `project.read` token 换取 |
| 数据隔离：跨 tenant/project、soft-delete、随机 UUID | `test_cross_tenant_isolation.py`、repository/API 真实 DB 测试、artifact/result/归档日志隐藏测试 | 已作为 PR integration 门禁；Member/Viewer 项目级路由先归一 404 再做 RBAC |
...
| API 写入后的真实 DB 状态 | projects、project members、pipelines、credentials、environments、notification rules、runs、schedules、auth/token、batch cancel/retry audit 链路 | 已补 required integration；项目 `git_url` userinfo、pipeline 复杂配置密钥、凭据明文、环境变量值、通知 channel/webhook/template 不落 audit before/after，project/pipeline/member/notification/schedule delete 有删除前快照 |
| Worker/queue 状态机与产物链路 | 真实 PG repository 状态机、run claim、cancel、state visibility、execute_run 基础设施异常重试、executor/task 单测、artifact size/count/recursive upload、S3 上传失败无孤儿 DB 行、下载链接 bucket/key/TTL/API token scope 真实 DB/API 测试、归档失败真实 Redis retry marker/TTL + worker cron 重试、归档日志默认页/分页/缺失对象/跨租户 404 API 测试、heavy Docker cancel、compose high/medium/low worker 配置、external-stack worker smoke 和 worker_lost retry 黑盒 | 已覆盖 queued/preparing/running/collecting/done/failed/cancelled/timeout；priority+FIFO 排序、worker retry 落库、artifact 递归落库、失败上传不落库、归档失败补偿和下载链接参数有真实 DB/API/RBAC/Redis 测试；artifact 预签名 URL 需要 `run.read` scope；nightly/manual 真实外部栈覆盖 worker 产物内容下载、归档日志读回和 worker_lost 后自动 retry |
