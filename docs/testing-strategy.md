...
| 风险面 | 当前证据 | 状态 |
| --- | --- | --- |
| RBAC：tenant role、project role、platform admin、API token scope | audit events、cross-tenant API、SSE 403/404、SSE ticket + `Last-Event-ID` 真实续传、artifact list/download 与 archived logs 的 `run.read` scope、真实 JWT/API token integration、auth 失败审计 outage 与 refresh revoke Redis blip 回归、register/refresh/logout/API token 审计防泄密 | 已覆盖主干；API token scope 已补真实路由矩阵，只读/错误/空 scope 都会被门禁，artifact 元数据列表、预签名 URL 和归档日志读回都不能由 `project.read` token 换取，tenant A `run.read` token 不能枚举 tenant B artifact；audit-events 成功查询写 `audit_events.list` 自审计，member/viewer 403 和跨租户过滤 404 不写误导性自审计；login/refresh/logout 在审计写入异常下不把 401/204 变成 500，refresh revoke Redis 抖动不阻断 token 轮换；register/refresh/logout 不把 password/access token/refresh token 写入 AuditEvent before/after，API token 创建/撤销审计只记录 name/scopes 等可追溯元数据 |
